using System.Drawing;

namespace L2RBot.Common
{
    public class Colors
    {
        public static Color White = Color.FromArgb(255, 255, 255, 255);

        public static Color Black = Color.FromArgb(255, 0, 0, 0);

        public static Color OffWhite = Color.FromArgb(255, 251, 251, 251);

        //Wifi Logo
        public static Color WifiLogo = Color.FromArgb(255, 79, 136, 42);

        //Quests colors identified at x value 16, y value 210-421(at the quest pane)
        public static Color MainQuest = Color.FromArgb(255, 255, 175, 0);

        public static Color ScrollQuest = Color.FromArgb(255, 75, 208, 247);

        public static Color WeeklyQuest = Color.FromArgb(255, 75, 154, 255);

        public static Color DailyQuest = Color.FromArgb(255, 80, 170, 30);

        public static Color WeeklyComplete = Color.FromArgb(255, 48, 85, 127);

        //Alter of Madness Quest Colors
        public static Color SpotReviveBtn = Color.FromArgb(255, 55, 91, 133);

        public static Color CompleteOk = Color.FromArgb(255, 58, 94, 135);

        public static Color CloseRechargeWindowX = Color.FromArgb(255, 162, 166, 173);

        public static Color CloseRechargeWindowCrystal = Color.FromArgb(255, 253, 154, 206);

        //Quest Helper
        public static Color StuckLoading = Color.FromArgb(255, 235, 235, 234);

        //Scroll Quest
        public static Color ScrollS = Color.FromArgb(255, 65, 175, 166);

        public static Color ScrollA = Color.FromArgb(255, 150, 182, 81);

        public static Color ScrollB = Color.FromArgb(255, 126, 166, 190);

        public static Color ScrollC = Color.FromArgb(255, 123, 123, 123);

        public static Color ScrollQuestMenuX = Color.FromArgb(255, 163, 167, 173);






    }
}
